  ...
  return (
    <>
      ...
      <section className="container-page py-16 sm:py-20">
        <div className="space-y-5">
          {services.map((service, index) => (
            <article
              key={service.slug}
              id={service.slug}
              className="reveal scroll-mt-24 rounded-2xl bg-white p-8 shadow-lift ring-1 ring-paper-200 sm:p-10"
            >
              <div className="grid gap-8 md:grid-cols-[auto_1fr_1fr] md:gap-10">
                <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-gold-50 text-navy-700 ring-1 ring-gold-100">
                ...
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-semibold tracking-tight text-navy-900">
                    {service.title}
                  </h2>
                  <p className="mt-3 text-pretty leading-relaxed text-navy-800/70">
                    {service.blurb}
                  </p>
                </div>
                ...
                <ul className="grid gap-3 self-center sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2">
                  {service.bullets.map((bullet) => (
                    <li
                      key={bullet}
                      className="flex items-start gap-2.5 text-sm text-navy-800/80"
                    >
        ...
        <div className="reveal mt-14 rounded-2xl border border-dashed border-paper-300 bg-white/60 p-9 text-center">
          <h2 className="text-xl font-semibold text-navy-900">
            Don&apos;t see what you need?
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-pretty leading-relaxed text-navy-800/70">
