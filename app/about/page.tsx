    ...
    <>
      ...
      <section className="container-page py-16 sm:py-20">
        <div className="grid gap-14 lg:grid-cols-[1.1fr_1fr] lg:gap-20">
          <div className="reveal space-y-5 text-pretty leading-relaxed text-navy-800/80">
            <p>
            ...
            <div className="!mt-9 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-2 rounded-full bg-navy-900 px-7 py-3.5 text-sm font-semibold text-paper-50 shadow-lift transition-all hover:bg-navy-800 active:scale-[0.98]"
              >
                Get in touch
          ...
          <div className="reveal space-y-5">
            {photos.length > 0 && (
              <div className="grid grid-cols-2 gap-3">
                <div className="relative col-span-2 aspect-[16/10] overflow-hidden rounded-2xl bg-paper-200 ring-1 ring-paper-200">
                  <Image
                    src={photos[0].src}
                    alt={photos[0].alt}
                    fill
                ...
                {photos.slice(1, 3).map((photo) => (
                  <div
                    key={photo.id}
                    className="relative aspect-square overflow-hidden rounded-2xl bg-paper-200 ring-1 ring-paper-200"
                  >
                    <Image
                      src={photo.src}
                      alt={photo.alt}
            ...
            <div className="rounded-2xl bg-white p-7 shadow-lift ring-1 ring-paper-200">
              <h2 className="flex items-center gap-2 font-semibold text-navy-900">
                <MapPinIcon className="h-4.5 w-4.5 text-navy-700" />
                Where we work
              </h2>
              <ul className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-navy-800/75">
                {site.areasServed.map((area) => (
                  <li key={area} className="flex items-start gap-2">
                    <CheckIcon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold-500" />
                    {area}
                  </li>
                ))}
              </ul>
              <p className="mt-5 border-t border-paper-200 pt-4 text-xs leading-relaxed text-navy-800/55">
